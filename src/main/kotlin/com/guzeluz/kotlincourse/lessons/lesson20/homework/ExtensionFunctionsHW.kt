package com.guzeluz.kotlincourse.lessons.lesson20.homework
...
/*2. Создайте функцию-расширение для изменяемого списка элементов:
    ...
    4. сам изменяемый список при этом должен стать отсортированным:
     по возрастанию, если в функцию передано true
...
/*4. Реализуйте метод расширения within для класса Number, который проверяет,
 ...
 other — число для сравнения и
 deviation — максимально допустимое отклонение.
 Метод должен возвращать true, если разница между текущим числом и
 числом для сравнения не превышает заданное отклонение.
...
fun Number.within(other: Number, deviation: Number): Boolean {
    val arg = toDouble() - other.toDouble()
    return arg.absoluteValue <= deviation.toDouble()
}

/*5. Реализуйте для класса String два метода расширения: encrypt и decrypt.
 Метод encrypt должен сдвигать каждый символ исходной строки на
 заданное число позиций вперед по таблице Unicode,
 ...
 Протестируйте вашу реализацию, убедившись, что после шифрования и
 последующей расшифровки строка возвращается к исходному состоянию.
 Сдвиг по таблице Unicode реализуется простым прибавлением или
...
выводить в консоли исходную строку побуквенно в верхнем регистре в столбик:
...
fun main() {
    ...
    println(arrayOf<Int>().fun1())

    println(mutableListOf(23, 4, 52, 5).fun2(true))
    println(mutableListOf(23, 4, 52, 5).fun2(false))

    println(mapOf(1 to listOf(1, 2), 2 to listOf(4, 5, 6), 3 to listOf(7, 8, 9, 10)).fun3(2))
    println(mapOf(1 to listOf(1, 2), 2 to listOf(4, 5, 6), 3 to listOf(7, 8, 9, 10)).fun3(3))

    println(1.within(0, 4))
    println(1.within(-5, 4))
    println(1.within(-1, 2))
