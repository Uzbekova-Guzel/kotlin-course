...
//    IntMotion()::move
//    /*Что такое ::provide? Это способ передачи функции в виде ссылки.
//    То-есть, тут не происходит вызов функции, она в этот момент не выполняется.
//    Тут мы сообщаем компилятору, что в качестве аргумента нужно использовать
//    вот это название функции. И так как её сигнатура соответствует сигнатуре
//    функции интерфейса, то она будет принята без ошибок.*/
//)

//способ написания кода функций непосредственно в виде аргументов класса.
...
//    println(a.performMove(2.0))

    val r = {
        Random.nextFloat().toString()
    }
    val parameterizedAuto = LambdaStringIntMobileParm(r)
    println(parameterizedAuto)
...
fun ex1(arg: String): (Int) -> String {
    val a: (Int) -> String = {
        (1..it).joinToString(" : ") { arg }
    }
    return a
//    return { (1 .. it).map { arg }.joinToString(" : ") { arg } }
}
